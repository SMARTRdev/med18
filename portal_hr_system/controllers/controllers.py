...
class CustomerPortalInherit(CustomerPortal):
    ...
    def list_method(self, page=1, sortby=None, filterby=None, search=None, search_in='all', groupby='none', **kw):
        ...
        if filterby == 'all':
            self.attendance_rec = request.env['hr.attendance'].sudo().search(
                [('employee_id', '=', omni_employee_id.id)])
        elif filterby == 'today':
...
class PortalLeaveKnk(CustomerPortal):
    ...
    def portal_payslip(self, page=1, sortby=None, filterby=None, search=None, search_in='all', groupby=None, **kw):
        ...
        allocations = request.env['hr.leave.allocation'].search(
            [('employee_id', '=', request.env.user.employee_id.id), ('state', '=', 'validate')])
        allocation_data = allocations.holiday_status_id.get_allocation_data(request.env.user.employee_id)
        allocation_data = allocation_data[request.env.user.employee_id]
    ...
    def apply_leave(self, **post):
        ...
        domain = ['|', ('requires_allocation', '=', 'no'), '&', ('has_valid_allocation', '=', True), '&',
                  ('virtual_remaining_leaves', '>', 0), ('max_leaves', '>', '0')]
    ...
    def save_leave(self, **post):
        ...
        attendances = request.env['resource.calendar.attendance'].read_group(domain_1, ['ids:array_agg(id)',
        ...
        attendance_from = next((att for att in attendances if int(att.dayofweek) >= start_date.weekday()),
                               attendances[0] if attendances else default_value)
        attendance_to = next((att for att in reversed(attendances) if int(att.dayofweek) <= end_date.weekday()),
                             attendances[-1] if attendances else default_value)
        hour_from = float_to_time(attendance_from.hour_from)
        hour_to = float_to_time(attendance_to.hour_to)
        start_date = timezone(employee.tz).localize(datetime.combine(start_date, hour_from)).astimezone(UTC).replace(
            tzinfo=None)
        end_date = timezone(employee.tz).localize(datetime.combine(end_date, hour_to)).astimezone(UTC).replace(
            tzinfo=None)
        error = False
        employee = request.env.user.employee_id
